...
const Register = () => {
  ...
  const serverError =
    registerMutation.data && !registerMutation.data.success
      ? registerMutation.data.error.message
      : '';
      ...
      <div className="w-full max-w-md">
        <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-gray-900/5">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700" htmlFor="name">Full name</label>
              <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Doe" className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 transition-colors focus:border-amazon-river focus:bg-white focus:ring-2 focus:ring-amazon-river/20 focus:outline-none" autoComplete="name" />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 8 characters" className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-2.5 pr-10 text-sm text-gray-900 placeholder:text-gray-400 transition-colors focus:border-amazon-river focus:bg-white focus:ring-2 focus:ring-amazon-river/20 focus:outline-none" autoComplete="new-password" />
              ...
              <ul className="mt-2 grid grid-cols-2 gap-1">
                {PASSWORD_RULES.map(({ test, label }) => (
                  <li key={label} className={`flex items-center gap-1.5 text-xs ${test(password) ? 'text-green-600' : 'text-gray-400'}`}>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700" htmlFor="passwordConfirm">Confirm password</label>
              <div className="relative">
                <input id="passwordConfirm" type={showPasswordConfirm ? 'text' : 'password'} value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} placeholder="Re-enter your password" className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-2.5 pr-10 text-sm text-gray-900 placeholder:text-gray-400 transition-colors focus:border-amazon-river focus:bg-white focus:ring-2 focus:ring-amazon-river/20 focus:outline-none" autoComplete="new-password" />
                <button type="button" onClick={() => setShowPasswordConfirm(!showPasswordConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" aria-label={showPasswordConfirm ? 'Hide password' : 'Show password'}>
                  <EyeIcon open={showPasswordConfirm} />
                </button>
              </div>
            </div>
            {error && <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">{error}</div>}
            <button type="submit" disabled={registerMutation.isPending} className="mt-2 w-full rounded-lg bg-amazon px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-amazon-light disabled:opacity-50">
